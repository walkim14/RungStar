...
`assets/fonts/` carries four TrueType faces, redistributed under the **SIL Open Font License
1.1**. The licence text sits beside them, which is what the OFL requires; neither face is
modified beyond being renamed, and neither is sold on its own.

| File | Face | Copyright |
|---|---|---|
| `RungStar-Regular.ttf` | Poppins Regular | The Poppins Project Authors — `OFL-Poppins.txt` |
| `RungStar-Bold.ttf` | Poppins SemiBold | as above |
| `RungStar-Lyrics.ttf` | Poppins Bold | as above |
| `RungStar-Fallback.ttf` | Fira Sans Regular | The Mozilla Foundation and Telefonica S.A. — `OFL-FiraSans.txt` |

The OFL is compatible with the GPL: it governs the font files themselves and places no
condition on the software that displays them.

## Bundled sounds

`assets/sounds/` is generated by `tools/make-sounds.py` and is original to this project, under
the same GPL-3.0-or-later as the rest of it. Nothing in it is sampled or adapted from anywhere.
